...
namespace WebProject.Controllers
{
    public class QuizController : Controller
    {
        ...
        public async Task<ActionResult> Question(ResponseModel responseModel, QuestionCategory questionCategory)
        {
            ...
            var model = CreateQuestionModel(question);
            model.HasSingleAnswer = question.Answers.Count == 2 || (CorrectAnswersIds.Count == 1 && !question.Answers.Any(a => a.Text.Trim().ToLower().Contains("all")) && !question.Answers.Any(a => a.Text.Trim().ToLower().Contains("none")));
            return View("Question", model);
        }
