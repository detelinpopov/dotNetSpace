$(document).ready(function() {   
    ...
    var showcaseJavaScript = $("#showcase-javascript");
    showcaseJavaScript.Cloud9Carousel({
        yPos: 42,
        yRadius: 48,
        mirrorOptions: {
            gap: 12,
        ...
        autoPlay: true,
        bringToFront: true,
        onRendered: showcaseUpdatedJs,
        onLoaded: function () {
            showcaseJavaScript.css("visibility", 'visible');
            showcaseJavaScript.css("display", 'none');
            showcaseJavaScript.fadeIn(1500);
        }
    });

    function showcaseUpdatedJs(showcaseJavaScript) {
        var title = $("#item-title").html(
            $(showcaseJavaScript.nearestItem()).attr("alt")
        );

        var c = Math.cos((showcaseJavaScript.floatIndex() % 1) * 2 * Math.PI);
        title.css("opacity", 0.5 + (0.5 * c));
    }
});